...
public class ViewModify {
	...
	public static void changeView(EntityPlayer player) {
		//Get current player pitch and yaw
		float playerPitch = player.rotationPitch;
		float playerYaw = player.rotationYaw;
				
		//Set to closest pitch and yaw axes
		while (ViewLock.keyBindings[0].isPressed()) {
		...
		while (ViewLock.keyBindings[1].isPressed()) {
			if(pitchLock) {
				pitchLock = false;
			} else {
				pitchLock = true;
		...
		//Toggle the yaw lock
		while (ViewLock.keyBindings[2].isPressed()) {
			if (yawLock) {
				yawLock = false;
			} else {
				yawLock = true;
