...
public class CEntityAnchorArgument implements ArgumentType<CEntityAnchorArgument.Anchor> {
	...
	private static final DynamicCommandExceptionType ERROR_INVALID = new DynamicCommandExceptionType(name -> new TranslatableComponent("argument.anchor.invalid", name));
	...
	public enum Anchor {
		...
		static final Map<String, Anchor> BY_NAME = Util.make(Maps.newHashMap(), map -> {
			Anchor[] var1 = values();
